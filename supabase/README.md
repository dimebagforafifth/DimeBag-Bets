...
| `0010_pools.sql` | Betting pools + user-created leagues: `pools` / `pool_entries` / `pool_picks` / `pool_invites` / `league_seasons`. **CREDIT** pools (entries/prizes flow through the same audited core path as every bet — these are config+state only, like `0007_import`, **not** fiat). Client-owned CRUD scoped to `owner = auth.uid()` with a `tenant_id`; member back-refs `on delete set null`; off-by-default. |
