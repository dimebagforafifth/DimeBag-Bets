...
| `0007_member_auth.sql` | Multi-user book auth (PR #10, from origin): `book_members` (auth user_id → book member, tenant-scoped) + operator-read RLS on `accounts` — backs Google OAuth + email verification + multi-user authz + server-side bet grading. Owner/operator-scoped reads; non-money (identity/authz only). |
| `0008_import.sql` | Operator player-import records: `import_batches` / `import_rows` / `import_mapping_templates`. Operator-authored, non-money config — client-owned CRUD scoped to `owner = auth.uid()` with a `tenant_id`. `player_id` is a back-reference to the created member, never a money write. (Renumbered from 0007 at the push reconcile to clear origin's `0007_member_auth.sql`.) |
| `0009_pricing_config.sql` | SGO pricing config: `pricing_config` — the operator's de-vig method + house margin (bps) as data, scoped global/sport/market. **Non-money** operator config (never moves a balance). Owner-scoped CRUD with a `tenant_id`; off-by-default (zero rows reproduces today's pricing exactly). |
| `0010_billing.sql` | **FIAT** per-head software billing: `billing_config` / `billing_periods` / `billing_head_snapshots`. The operator's real-money cost (per active player/week) — integer-cents dollars, **NOT** the player figure, **NOT** the ledger, never the `0003` money RPCs. Operator-owned CRUD scoped to `owner = auth.uid()` with a `tenant_id`; off-by-default. |
| `0011_player_profile.sql` | Player-profile READ-ONLY projection + social graph: `player_profile_stats_mv` (the materialised windowed stats), `follow_edge` (the sport-scoped follow edges; 'all' edges live in social), `profile_privacy` (per-block visibility). **Non-money** — pure projection of the audited ledger, mints nothing. `player_profile_stats_mv` is client-read-only (read-own RLS); `follow_edge`/`profile_privacy` are owner-scoped CRUD. Off-by-default. |
| `0012_pools.sql` | Betting pools + user-created leagues: `pools` / `pool_entries` / `pool_picks` / `pool_invites` / `league_seasons`. **CREDIT** pools (entries/prizes flow through the same audited core path as every bet — these are config+state only, like `0008_import`, **not** fiat). Client-owned CRUD scoped to `owner = auth.uid()` with a `tenant_id`; member back-refs `on delete set null`; off-by-default. |
| `0013_player_limits.sql` | Responsible-play self-limits: `player_limits` (a player's own wager/loss caps, cool-off, soft session). **Non-money** — config only; the limit GATE lives in `core.placeWager` (`assertWithinLimits`, no-op when unset → default placement byte-identical), never a balance write. Owner-scoped CRUD with a `tenant_id`; off-by-default. |
| `0014_referrals.sql` | Referral loops: `referral_config` (program on/off + reward) + `referrals` (one row per invite: referrer/referee/code/status). **Config + state only** — the reward is issued through the existing audited `core.grant` path (`mutateBook` → `recordIssuance('referral')`), never a parallel ledger; off-by-default (program ships disabled). Owner-scoped CRUD with a `tenant_id`; one-per-referee partial-unique guard against abuse. |

> The `0008–0014` chain was renumbered (+1) at the push reconcile so origin's `0007_member_auth.sql` (PR #10) keeps slot 0007; the tables are independent, so order between `0007_member_auth` and `0008_import` is immaterial.
