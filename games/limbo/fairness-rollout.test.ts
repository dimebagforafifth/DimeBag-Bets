...
import { playLimbo, verifyLimbo } from './index.js'
...
describe('limbo — server seed comes from the fairness authority', () => {
  it('plays with an authority-minted seed; the round seed IS the committed one and verifies', async () => {
    ...
    expect(verifyLimbo(round.serverSeed, round.clientSeed, round.nonce, round.result)).toBe(true)
  })
})
