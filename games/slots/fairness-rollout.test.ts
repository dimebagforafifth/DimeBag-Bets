...
import { playSlots, verifySpin } from './index.js'
...
describe('slots — server seed comes from the fairness authority', () => {
  it('spins with an authority-minted seed; the round seed IS the committed one and verifies', async () => {
    ...
    // fairness math + verify tooling unchanged — the reels re-derive from the revealed seed
    expect(verifySpin(round.serverSeed, round.clientSeed, round.nonce, round.reels)).toBe(true)
  })
})
