 ...
 * book member id, so every player's status/spendable/bonuses survive a reload and the
 * operator can report on them.
 *
 * COINS / STATUS ONLY — four kinds of value, none of them cash or withdrawable:
 *   - STATUS    — monotonic points that drive the tier ladder. Only ever goes up.
 *   - SPENDABLE — the rewards currency, earned from play (cashback) + daily/missions/
 *                 contests/comps, redeemed in the store. Never cash.
 *   - CASHBACK  — accrues as a % of coins WAGERED; claimed into spendable.
 *   - LOCKED    — bonus coins from promos/top-ups/comps that unlock to REGULAR coins
 *                 after a wagering requirement (the coins-only playthrough — NEVER a
 *                 cash-out condition). Unlocks are returned for the host to credit.
 *
 * "Regular coins" (the betting balance) live in the shared core account, not here; this
 * module returns the coin delta to credit when a locked bonus matures or cashback/store
 * pays out, and the host moves it through core.
 */
...
export interface LockedBonus {
  id: string
  /** Coins that unlock to the regular balance once cleared. */
  amount: number
  /** Coins that must be wagered to unlock (amount × promo playthrough). */
  wagerRequired: number
  /** Coins wagered toward the requirement so far. */
  wagered: number
  /** Where it came from (a promo name, a comp, a top-up). */
  source: string
}

export type CompKind = 'coins' | 'freeplay' | 'limitboost' | 'badge'
export interface CompRecord {
  ...
  byName: string
  kind: CompKind
  /** Coins (for 'coins'/'freeplay' stake) or 0 for badge/limit. */
  amount: number
  reason: string
  at: number
}
...
export interface PlayerRewardState {
  ...
  spendable: number // rewards currency (store)
  cashbackPending: number // accrued, claim → spendable
  locked: LockedBonus[]
...
const SEED: Record<string, PlayerRewardState> = {
  'p-marco': {
    ...
    compHistory: [
      { id: 'c1', by: 'a-e', byName: 'East Desk', kind: 'coins', amount: 1_000, reason: 'Loyalty — rough week', at: 1_749_900_000_000 },
    ],
    claimedIds: [],
    dailyDay: 5,
    dailyClaimedToday: false,
  ...
  'p-tariq': {
    ...
    compHistory: [
      { id: 'c2', by: 'a-w', byName: 'West Desk', kind: 'coins', amount: 2_000, reason: 'Welcome back bonus', at: 1_749_950_000_000 },
    ],
    claimedIds: [],
    dailyDay: 1,
    dailyClaimedToday: false,
  ...
  'p-dana': {
    ...
    compHistory: [
      { id: 'c3', by: 'mgr', byName: 'Operator', kind: 'coins', amount: 10_000, reason: 'VIP host — Diamond care', at: 1_749_990_000_000 },
 ...
 * configured rate, and every locked bonus advances toward its playthrough. Returns the
 * coins that just UNLOCKED to the regular balance (the host credits them through core).
 */
export function accrueFromWager(memberId: string, stakeCoins: number): { unlockedCoins: number } {
  if (stakeCoins <= 0) return { unlockedCoins: 0 }
  const rate = getRewardsConfig().economy.cashbackRate
  let unlocked = 0
  mutate(memberId, (s) => {
    const locked: LockedBonus[] = []
    for (const b of s.locked) {
      const wagered = b.wagered + stakeCoins
      if (wagered >= b.wagerRequired) unlocked += b.amount
      else locked.push({ ...b, wagered })
    }
    return {
      ...s,
      status: s.status + stakeCoins,
      cashbackPending: s.cashbackPending + Math.round(stakeCoins * rate),
      locked,
    }
  })
  return { unlockedCoins: unlocked }
}

/** Claim accrued cashback into the spendable rewards balance. Returns coins moved. */
export function claimCashback(memberId: string): number {
  const s = getPlayerRewards(memberId)
  const amt = s.cashbackPending
  if (amt <= 0) return 0
  set(memberId, { ...s, cashbackPending: 0, spendable: s.spendable + amt })
  return amt
}

/** Credit spendable rewards (daily / mission / contest payout). */
export function addSpendable(memberId: string, coins: number): void {
  if (coins === 0) return
  mutate(memberId, (s) => ({ ...s, spendable: Math.max(0, s.spendable + coins) }))
}

/** Spend from the spendable balance (store redemption). Returns false if short. */
export function spendSpendable(memberId: string, coins: number): boolean {
  const s = getPlayerRewards(memberId)
  if (coins <= 0 || s.spendable < coins) return false
  set(memberId, { ...s, spendable: s.spendable - coins })
  return true
}

/** Grant locked bonus coins (a promo / top-up / comp). They unlock to regular coins
 *  only after `amount × playthrough` coins are wagered. playthrough 0 → unlocks instantly
 *  (returned for immediate credit). Returns coins to credit now (0 unless instant). */
export function grantLockedBonus(
  ...
  if (amount <= 0) return { instantCoins: 0 }
  if (playthrough <= 0) return { instantCoins: amount } // no lock — host credits regular coins
  mutate(memberId, (s) => ({
    ...s,
    locked: [...s.locked, { id, amount, wagerRequired: amount * playthrough, wagered: 0, source }],
  }))
  return { instantCoins: 0 }
}
 ...
 *  Coins comps also credit spendable; locked-bonus comps go through grantLockedBonus. */
