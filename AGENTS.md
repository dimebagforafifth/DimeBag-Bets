...
*(AGENTS.md — place at the repo root. Codex reads this automatically as standing context. Read this before any task; it is the single source of truth for what we're building and how.)*
...
## 0. How to use this file (for Codex)
...
- **Phase 2 — Roll-up & interface (JD leads heavier Codex work):** bring modules under one app shell (one at a time), build the clean interface (Home, Game view, Account, games hub). One login, one balance across everything.
...
**Team:** "You" build the Phase 0 groundwork solo now. **JD** (more fluent in Codex) comes home in ~1.5 weeks and leads the heavier build from Phase 1 on.
