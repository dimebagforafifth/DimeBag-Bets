 ...
 * module is the single place that RESOLVES them (the dual-runtime physical read is
 * centralized in `lib/env` and reused here), so every other file just asks
 ...
 *
 * Env is read from both worlds the app runs in (see `lib/env` `readEnv`):
